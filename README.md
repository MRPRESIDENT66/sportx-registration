...
这是一个从 SportX 运动平台中独立提炼的高并发限量报名服务。项目只保留报名、可靠事件投递和失败恢复链路，便于清晰验证并发安全和消息可靠性，而不是直接提交完整业务平台。
...
初始化活动 `10001` 包含 50 个名额；`loadtest/register.lua` 会轮询 100 个不同用户。
...
预期不变量：`joined_slots = 50`、报名数 `= 50`、不同用户数 `= 50`、最后一个重复查询无结果。提交作业时补充原始 `wrk` 输出，以及这些 SQL、RabbitMQ 管理台、失败消息回放的截图即可。
